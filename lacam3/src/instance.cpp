...
if (start.first < 0 || G->height <= start.first || 
start.second < 0 || G->width <= start.second) {
info(1, 0, "Invalid start node coordinates");
return;
}
int index = G->width * start.first + start.second;
// printf("Index: %d\n", index);
...
if (goal.first < 0 || G->height <= goal.first || 
goal.second < 0 || G->width <= goal.second) {
info(1, 0, "Invalid goal node coordinates");
return;
}
int index = G->width * goal.first + goal.second;
// printf("Index: %d\n", index);
if (G->U[index]!= nullptr) {
